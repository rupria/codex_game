-- JokerHandChoice 0.5.9 for GitHub issue #70.
-- Produces a compact opaque modal with a single vertical option list.
...
local function optionButton(state)
  local out=img(440,44); local y=(state=="pressed") and 2 or 0
  fill(out,7,y+4,426,36,C.shadow)
  poly(out,{{3,y+7},{10,y+1},{430,y+1},{437,y+7},{437,y+36},{430,y+42},{10,y+42},{3,y+36}},C.ink)
  poly(out,{{7,y+8},{12,y+4},{428,y+4},{433,y+8},{433,y+35},{428,y+39},{12,y+39},{7,y+35}},C.brass0)
  fill(out,12,y+7,416,29,C.leather)
  frame(out,16,y+9,408,25,C.brass1,1)
  bolt(out,12,y+7); bolt(out,428,y+7); bolt(out,12,y+35); bolt(out,428,y+35)
  if state=="hover" then
    frame(out,12,y+6,416,31,C.brass3,2); lineH(out,38,y+10,364,C.brass2,1)
  elseif state=="selected" then
    frame(out,11,y+5,418,33,C.cyan,2); lineH(out,44,y+36,352,C.cyan,2)
  elseif state=="disabled" then
    fill(out,10,y+5,420,34,Color{r=4,g=4,b=4,a=148}); frame(out,16,y+9,408,25,C.muted,1)
  end
  return out
end
...
local panel=nearest(loadImage(panelPath),600,420)
...
panel:saveAs(runtimeRoot.."/joker_hand_choice_panel_compact_600x420_0_5_9.png")
dim:saveAs(runtimeRoot.."/joker_hand_choice_dim_960x540_0_5_9.png")
idle:saveAs(runtimeRoot.."/joker_hand_option_idle_440x44_0_5_9.png")
hover:saveAs(runtimeRoot.."/joker_hand_option_hover_440x44_0_5_9.png")
selected:saveAs(runtimeRoot.."/joker_hand_option_selected_440x44_0_5_9.png")
disabled:saveAs(runtimeRoot.."/joker_hand_option_disabled_440x44_0_5_9.png")

local before=nearest(loadImage(beforePath),960,540)
local preview=before:clone(); preview:drawImage(dim,Point(0,0)); preview:drawImage(panel,Point(180,60))
text(preview,"JOKER HAND",480,198,3,C.cream)
text(preview,"CHOOSE ONE AVAILABLE HAND",480,220,1,C.muted)
local labels={"HIGH CARD","ONE PAIR","TWO PAIR","THREE OF A KIND"}
for i,label in ipairs(labels) do
  local x=260; local y=236+(i-1)*46
  local state=(i==2) and hover or ((i==4) and selected or idle)
  preview:drawImage(state,Point(x,y)); text(preview,label,x+220,y+18,1,C.cream)
end
preview:saveAs(previewRoot.."/joker_hand_choice_vertical_4_preview_960x540_0_5_9.png")
...
for i,pair in ipairs(stateList) do
  local col=(i-1)%2; local row=math.floor((i-1)/2); local x=20+col*480; local y=24+row*108
  states:drawImage(pair[1],Point(x,y)); text(states,pair[2],x+220,y+18,1,C.cream)
end
states:saveAs(previewRoot.."/joker_hand_option_states_960x260_0_5_9.png")

local panelSrc=Sprite(600,420,ColorMode.RGB); panelSrc.layers[1].name="compact_opaque_joker_hand_panel"
panelSrc.cels[1].image:drawImage(panel,Point(0,0))
local buttonSrc=Sprite(440,44,ColorMode.RGB); buttonSrc.layers[1].name="joker_hand_option_states"
...
panelSrc:saveAs(sourceRoot.."/joker_hand_choice_panel_compact_600x420_0_5_9.aseprite"); panelSrc:close()
buttonSrc:saveAs(sourceRoot.."/joker_hand_option_states_440x44_0_5_9.aseprite"); buttonSrc:close()
