...
- 주머니 내부는 둥근 열린 포켓이며 사각 테두리, 패널 모서리, 탄창 슬롯을 그리지 않습니다.
- `0~5`: 큰 총알 `24×40`을 모두 가로·사선으로 눕히고 서로 다른 X/Y/회전값으로 겹쳐 표시합니다.
- `6~30`: 작은 총알 `10×18`도 가로 방향을 유지한 채 행마다 오프셋과 회전을 달리해 쌓습니다. 정사각 격자를 사용하지 않습니다.
...
- [ ] 주머니 입구에 사각 프레임이나 세로 슬롯이 없다.
- [ ] 주머니 안 총알이 모두 눕혀져 작은 더미로 보인다.
